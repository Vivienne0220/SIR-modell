...
def run():
    ...
    bg_handles = [
        Patch(facecolor="lightgray", edgecolor="black", linewidth=0.8, alpha=0.25, label="Kezdeti időszak"),
        Patch(facecolor="orange", edgecolor="black", linewidth=0.8, alpha=0.25, label="Kijárási tilalom"),
        Patch(facecolor="green", edgecolor="black", linewidth=0.8, alpha=0.25, label="Oltási időszak")
    ]
    ...
    def add_period_backgrounds_single(x, lockdown, vaccine):
        ...
        for ax in axs.flat:
            ...
            ax.axvline(start, linestyle="--", color="black", linewidth=1.0, alpha=0.7)
            ax.axvline(lock, linestyle="--", color="black", linewidth=1.0, alpha=0.7)
            ax.axvline(vac, linestyle="--", color="black", linewidth=1.0, alpha=0.7)
            ax.axvline(end, linestyle="--", color="black", linewidth=1.0, alpha=0.7)
    ...
    def plot():
        try:
            ...
            if country == "Nitrai Kerület":
                ...
                axs[0, 0].plot(x, positive, color="tab:blue", linewidth=LINE_WIDTH)
                axs[0, 0].set_title("Fertőzések / pozitív teszt")

                axs[0, 1].plot(x, deaths, color="black", linewidth=LINE_WIDTH)
                axs[0, 1].set_title("Halálozás")

                axs[1, 0].plot(x, recovered, color="tab:green", linewidth=LINE_WIDTH)
                axs[1, 0].plot(x, active, color="purple", linewidth=LINE_WIDTH)
                axs[1, 0].set_title("SEIRDVH becslés")
                axs[1, 0].legend(handles=[
                    Patch(facecolor="tab:green", edgecolor="black", label="Becsült gyógyult"),
                    Patch(facecolor="purple", edgecolor="black", label="Becsült aktív")
                ], frameon=False)

                axs[1, 1].plot(x, hospital, color="tab:orange", linewidth=LINE_WIDTH)
                axs[1, 1].plot(x, vaccines, color="teal", linewidth=LINE_WIDTH)
                axs[1, 1].set_title("Kórház / oltás")
                axs[1, 1].legend(handles=[
                    Patch(facecolor="tab:orange", edgecolor="black", label="Kórház"),
                    Patch(facecolor="teal", edgecolor="black", label="Oltás")
                ], frameon=False)
                ...
                axs[0, 0].plot(x, infections, color="tab:red", linewidth=LINE_WIDTH)
                ...
                axs[1, 1].plot(x, hospital, color="tab:orange", linewidth=LINE_WIDTH)
                axs[1, 1].plot(x, vaccines, color="purple", linewidth=LINE_WIDTH)
                axs[1, 1].set_title("Kórház / Napi oltás")
                axs[1, 1].legend(handles=[
                    Patch(facecolor="tab:orange", edgecolor="black", label="Kórház"),
                    Patch(facecolor="purple", edgecolor="black", label="Oltás")
                ], frameon=False)
    ...
    def compare():
        try:
            ...
            hu_cases = smooth(get_daily_series(hu, "newcases", hu_cum) / pop_hu * 100000)
            sk_cases = smooth(get_daily_series(sk, "newcases", sk_cum) / pop_sk * 100000)

            hu_deaths = smooth(get_daily_series(hu, "death", hu_cum) / pop_hu * 100000)
            sk_deaths = smooth(get_daily_series(sk, "death", sk_cum) / pop_sk * 100000)

            hu_rec = smooth(get_daily_series(hu, "recovered", hu_cum) / pop_hu * 100000)
            sk_rec = smooth(get_daily_series(sk, "recovered", sk_cum) / pop_sk * 100000)

            hu_hosp = smooth(get_level_series(hu, "hospital") / pop_hu * 100000)
            sk_hosp = smooth(get_level_series(sk, "hospital") / pop_sk * 100000)

            axs[0, 0].plot(hu["date"], hu_cases, color="tab:red", linewidth=LINE_WIDTH)
            axs[0, 0].plot(sk["date"], sk_cases, color="tab:blue", linewidth=LINE_WIDTH)
            axs[0, 0].set_title("Új fertőzések")
            axs[0, 0].set_ylabel("Esetszám / 100 000 lakos")
            axs[0, 0].legend(handles=[
                Patch(facecolor="tab:red", edgecolor="black", label="Magyarország"),
                Patch(facecolor="tab:blue", edgecolor="black", label="Szlovákia")
            ], frameon=False)

            axs[0, 1].plot(hu["date"], hu_deaths, color="tab:red", linewidth=LINE_WIDTH)
            axs[0, 1].plot(sk["date"], sk_deaths, color="tab:blue", linewidth=LINE_WIDTH)
            axs[0, 1].set_title("Halálozás")
            ...
            axs[1, 0].plot(hu["date"], hu_rec, color="tab:red", linewidth=LINE_WIDTH)
            axs[1, 0].plot(sk["date"], sk_rec, color="tab:blue", linewidth=LINE_WIDTH)
            ...
            axs[1, 1].plot(hu["date"], hu_hosp, color="tab:red", linewidth=LINE_WIDTH)
            axs[1, 1].plot(sk["date"], sk_hosp, color="tab:blue", linewidth=LINE_WIDTH)
    ...
    ttk.Button(left, text="Kimutatás", command=plot, width=22).pack(pady=10)
    ttk.Button(left, text="HU vs SK összehasonlítás", command=compare, width=22).pack(pady=10)
